...
async function loadSubjects() {
    ...
    try {
        const response = await fetch('http://localhost:8000/api/admin/subjects/listar', {
            method: 'GET',
            headers: {
...
function renderSubjects(subjectsToRender) {
    ...
    subjectsList.innerHTML = subjectsToRender.map(subject => `
...
async function handleAddSubject(e) {
    ...
    try {
        const response = await fetch('http://localhost:8000/api/admin/subjects/cadastrar', {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${adminToken}`,
                'Content-Type': 'application/json'
        ...
        if (response.ok) {
            ...
            showMessage(messageDiv, data.message || 'Erro ao adicionar matéria.', 'error');
        }

    } catch (error) {
        console.error('Erro ao adicionar matéria:', error);
        showMessage(messageDiv, 'Erro ao adicionar matéria. Tente novamente.', 'error');
    }
}
...
async function handleEditSubject(e) {
    ...
    try {
        const response = await fetch(`http://localhost:8000/api/admin/subjects/atualizar/${currentSubjectId}`, {
            method: 'PUT',
            headers: {
                'Authorization': `Bearer ${adminToken}`,
                'Content-Type': 'application/json'
        ...
        if (response.ok) {
            ...
            showMessage(messageDiv, data.message || 'Erro ao atualizar matéria.', 'error');
        }

    } catch (error) {
        console.error('Erro ao atualizar matéria:', error);
        showMessage(messageDiv, 'Erro ao atualizar matéria. Tente novamente.', 'error');
    }
}
...
async function confirmDelete() {
    ...
    try {
        const response = await fetch(`http://localhost:8000/api/admin/subjects/deletar/${currentSubjectId}`, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${adminToken}`,
                'Content-Type': 'application/json'
        ...
        if (response.ok) {
            ...
            showMessage(messageDiv, data.message || 'Erro ao excluir matéria.', 'error');
        }

    } catch (error) {
        console.error('Erro ao excluir matéria:', error);
        showMessage(messageDiv, 'Erro ao excluir matéria. Tente novamente.', 'error');
    }
}
